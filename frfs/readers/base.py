...
from frfs.nputil import fuzzysort
...
class BaseReader(object, metaclass=ABCMeta):
    ...
    @abstractmethod
    def _to_raw_frfsm(self):
        pass

    def to_frfsm(self):
        mesh = self._to_raw_frfsm()
...
class NodalMeshAssembler(object):
    ...
    _petype_focount = {'point': 1, 'line': 2, 'tri': 3, 'quad': 4,
    ...
    def _check_pyr_parallelogram(self, foeles):
        ...
        # Find FRFS node map for the quad face
        fnmap = self._petype_fnmap['pyr']['quad'][0]
        pfnmap = self._nodemaps.from_frfs['quad', 4][fnmap]
    ...
    def get_shape_points(self):
        ...
        for etype, pent in self._elenodes:
            ...
            # Go from Gmsh to FRFS node ordering
            peles = eles[:, self._nodemaps.from_frfs[petype, nnodes]]
