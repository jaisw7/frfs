...
"""Mappings between the ordering of FRFS nodes, and those of external formats
...
class GmshNodeMaps(object):
    ...
    Gmsh node ordering to FRFS, and the other provides the inverse.
    ...
    via their list index. When lists in the "gmsh_to_frfs" dictionary
    are indexed using the Gmsh node number, they return the equivalent
    FRFS node number.  The reverse is true for the "frfs_to_gmsh"
    dictionary.

    :Example: Convert Gmsh node number 4, in a 64 point hexahedra, to
              the equivalent node number in FRFS:

    >>> from frfs.readers.nodemaps import GmshNodeMaps
    >>> GmshNodeMaps.gmsh_to_frfs['hex', 64][4]
    >>> 48
    """
    to_frfs = {
        ...
        ('line', 2): np.array([0, 1])  # added for 1D
    }

    from_frfs = {k: np.argsort(v) for k, v in to_frfs.items()}
