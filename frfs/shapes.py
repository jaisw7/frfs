...
from frfs.nputil import block_diag, clean
from frfs.polys import get_polybasis
from frfs.quadrules import get_quadrule
from frfs.util import lazyprop
...
class BaseShape(object):
    ...
    @lazyprop
    def m12(self):
        m = np.matmul(self.ubasis.vdm.T, self.ubasis.vdm)
        return m.reshape(self.nupts, -1)
    ...
    def gbasis_coeffs(self):
        ...
        #qrule_map = {
        #    'point': ('gauss-legendre', 1),
        #   'line': ('gauss-legendre', int(1.5*(self.order + 1))),
        #    'quad': ('gauss-legendre', int((1.5*(self.order + 1))**2)),
        #    'tri': ('williams-shunn', 36)
        #}
        ...
        for kind, proj, norm in self.faces:
            gbqrule = qrule_map[kind]
            sect = 'solver-{0}-{1}'.format('interfaces', kind)
            if self.cfg.hasopt(sect, 'gbasis-pts'):
                gbqrule = (self.cfg.get(sect, 'gbasis-pts'), gbqrule[1])
