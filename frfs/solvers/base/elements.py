...
from frfs.nputil import npeval, fuzzysort
from frfs.util import lazyprop, memoize
...
class BaseElements(object, metaclass=ABCMeta):
    ...
    @memoize
    def opmat_raw(self, expr):
        return self.basis.opmat(expr)
    ...
    def rcpdjac_at_np(self, name):
        ...
        print("Jac (min: %s, max: %s)"%(np.min(djac), np.max(djac)))
    ...
    @memoize
    def djac_at(self, name):
        return self._be.const_matrix(self.djac_at_np(name), tags={'align'})
