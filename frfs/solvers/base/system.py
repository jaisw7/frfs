...
from frfs.util import proxylist, subclasses
...
class BaseSystem(object, metaclass=ABCMeta):
    ...
    def __init__(self, backend, rallocs, mesh, initsoln, nreg, cfg, **kwargs):
        ...
        # Load the interfaces
        int_inters = self._load_int_inters(rallocs, mesh, elemap, **kwargs)
        mpi_inters = self._load_mpi_inters(rallocs, mesh, elemap, **kwargs)
        self.bc_inters = self._load_bc_inters(rallocs, mesh, elemap, **kwargs)
        backend.commit()

        # Prepare the queues and kernels
        self._gen_queues()
        self._gen_kernels(eles, int_inters, mpi_inters, self.bc_inters)
        backend.commit()
