...
# The following modules are based on the "FRFS" implementation 
# (See licences/LICENSE_FRFS)

from ctypes import POINTER, c_int, c_double, c_float, c_void_p, c_long
import numpy as np
import ctypes
import ctypes.util
import os
...
def load_library(name):
    ...
    lpath = os.environ.get('FRFS_{0}_LIBRARY_PATH'.format(name.upper()))
    if lpath:
...
class CUBLASWrappers(object):
    ...
    def __init__(self):
        ...
        self.cublasDgemmStridedBatched = lib.cublasDgemmStridedBatched
        self.cublasDgemmStridedBatched.argtypes = [
            c_void_p, c_int, c_int, c_int, c_int, c_int,
            POINTER(c_double), 
            c_void_p, c_int, c_long,
            c_void_p, c_int, c_long,            
            POINTER(c_double), 
            c_void_p, c_int, c_long, c_int
        ]
        self.cublasDgemmStridedBatched.errcheck = self._errcheck
...
class CUDACUBLASKernels(object):
    ...
    # highly specialized without any bounds check
    def mulp(self, a, sA, b, sB, c, sC, alpha=1.0, beta=0.0):
        w = self._wrappers
    ...
    def mul__(self, a, b, out, alpha=1.0, beta=0.0):
        w = self._wrappers

        print(list(map(lambda v: (v.nrow, v.ncol), (a, b, out))))
