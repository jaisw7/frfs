...
class DGFSInitCondition(object, metaclass=ABCMeta):
    def __init__(self, backend, cfg, velocitymesh, name):
    ...
    def apply_init_vals(self, scal_upts_full, ele):
        initvals = self._init_vals
        for j in range(self.vm.vsize()):
            scal_upts_full[:, j, :] = initvals[j]


class DGFSMaxwellianInitCondition(DGFSInitCondition):
    model = 'maxwellian'

    def __init__(self, backend, cfg, velocitymesh, name):
        super().__init__(backend, cfg, velocitymesh, name)

    def load_parameters(self, name):
        u0 = self.vm.u0()
        self._rhoini = self.cfg.getfloat(name, 'rho')/self.vm.rho0()
...
class DGFSBKWInitCondition(DGFSInitCondition):
    model = 'bkw'
    ...
    def load_parameters(self, name):
        self._t0 = self.cfg.getfloat(name, 't0')
...
class DGFSMaxwellianExprInitCondition(DGFSInitCondition):
    ...
    def apply_init_vals(self, scal_upts_full, ele):
        ...
        rhoini = npeval(self.cfg.getexpr(self.name,'rho'),vars)/self.vm.rho0()
